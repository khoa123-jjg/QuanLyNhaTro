...
public class AdminController : Controller
{
    ...
    [HttpGet]
    public async Task<IActionResult> ChinhSuaNguoiDung(int id)
    {
        ViewData["Title"] = "Chỉnh sửa người dùng";
        ViewData["ActiveAdminMenu"] = "NguoiDung";

        var model = await _adminNguoiDungRepository.GetChinhSuaNguoiDungAsync(id);
        if (model is null)
        {
            TempData["Error"] = "Không tìm thấy người dùng.";
            return RedirectToAction(nameof(NguoiDung));
    ...
    public async Task<IActionResult> ChinhSuaNguoiDung(AdminNguoiDungEditViewModel model)
    {
        ViewData["Title"] = "Chỉnh sửa người dùng";
        ViewData["ActiveAdminMenu"] = "NguoiDung";
        ...
        {
            var reload = await _adminNguoiDungRepository.GetChinhSuaNguoiDungAsync(model.Id);
            if (reload is null)
            {
                TempData["Error"] = "Không tìm thấy người dùng.";
                return RedirectToAction(nameof(NguoiDung));
            }
            reload.HoTen = model.HoTen;
            reload.Email = model.Email;
            reload.SoDienThoai = model.SoDienThoai;
            reload.TrangThai = model.TrangThai;
            reload.GhiChu = model.GhiChu;
            reload.NgheNghiep = model.NgheNghiep;
            reload.NhuCauThue = model.NhuCauThue;
            reload.TrangThaiHoSo = model.TrangThaiHoSo;
            return View(reload);
        }

        var result = await _adminNguoiDungRepository.ChinhSuaNguoiDungAsync(model);
        if (result.Success)
        {
            TempData["Success"] = result.Message;
            return RedirectToAction(nameof(ChiTietNguoiDung), new { id = model.Id });
        }

        TempData["Error"] = result.Message;
        var failedReload = await _adminNguoiDungRepository.GetChinhSuaNguoiDungAsync(model.Id);
        if (failedReload is not null)
        {
            failedReload.HoTen = model.HoTen;
            failedReload.Email = model.Email;
            failedReload.SoDienThoai = model.SoDienThoai;
            failedReload.TrangThai = model.TrangThai;
            failedReload.GhiChu = model.GhiChu;
            failedReload.NgheNghiep = model.NgheNghiep;
            failedReload.NhuCauThue = model.NhuCauThue;
            failedReload.TrangThaiHoSo = model.TrangThaiHoSo;
            return View(failedReload);
        }
        return RedirectToAction(nameof(NguoiDung));
    }
    ...
    [HttpGet]
    public async Task<IActionResult> QuanHuyen(string? tuKhoa, int? id)
    {
        ViewData["ActiveAdminMenu"] = "QuanHuyen";
        var model = await _adminDiaChiRepository.GetQuanHuyenPageAsync(tuKhoa, id);
        return View(model);
    }

    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> LuuQuanHuyen(QuanHuyenFormViewModel form)
    {
        if (!ModelState.IsValid)
        {
            var model = await _adminDiaChiRepository.GetQuanHuyenPageAsync(null, form.Id);
            model.Form = form;
            return View("QuanHuyen", model);
        }

        var result = await _adminDiaChiRepository.LuuQuanHuyenAsync(form);
        TempData[result.Success ? "Success" : "Error"] = result.Message;
        return RedirectToAction(nameof(QuanHuyen));
    }
    ...
    {
        var result = await _adminDiaChiRepository.XoaQuanHuyenAsync(id);
        TempData[result.Success ? "Success" : "Error"] = result.Message;
        return RedirectToAction(nameof(QuanHuyen));
    }
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> CapNhatHoSoAdmin(AdminHoSoViewModel model)
    {
        ViewData["Title"] = "Hồ sơ admin";
        ViewData["ActiveAdminMenu"] = "HoSo";
        ...
        ModelState.Remove("DoiMatKhau.MatKhauHienTai");
        ModelState.Remove("DoiMatKhau.MatKhauMoi");
        ModelState.Remove("DoiMatKhau.XacNhanMatKhauMoi");
        ...
        if (!ModelState.IsValid)
        {
            ...
            pageModel.HoTen = model.HoTen;
            pageModel.Email = model.Email;
            pageModel.SoDienThoai = model.SoDienThoai;
            pageModel.GhiChu = model.GhiChu;
            return View("HoSo", pageModel);
        }

        var result = await _adminTaiKhoanRepository.CapNhatHoSoAsync(userId, model);
        if (result.Success)
        {
            TempData["Success"] = result.Message;
        }
